...
class PaymentScreen extends StatefulWidget {
  ...
  const PaymentScreen({super.key, required this.totalHarga, required this.keranjang});
...
class _PaymentScreenState extends State<PaymentScreen> {
  ...
  void _terimaPembayaran() async {
    final int uangDiterima = int.tryParse(_uangDiterimaController.text.replaceAll('.', '').replaceAll(',', '')) ?? 0;
    if (uangDiterima < widget.totalHarga) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Uang yang diterima kurang')));
      return;
    }
    ...
    final List<Map<String, dynamic>> finalKeranjangData = List.from(widget.keranjang);

    try {
      // 1. Update stok in Firestore
      WriteBatch batch = FirebaseFirestore.instance.batch(); // Use batch for atomic writes
      for (var item in finalKeranjangData) {
        ...
        if (docId != null && jumlahDibeli > 0) {
          var docRef = FirebaseFirestore.instance.collection('produk').doc(docId);
          ...
          // Consider fetching all product stocks beforehand if performance is critical.
           DocumentSnapshot snapshot = await docRef.get(); // Consider error handling if doc doesn't exist
           if (snapshot.exists) {
              final currentStok = (snapshot.data() as Map<String, dynamic>?)?['stok'] as int? ?? 0;
              int newStok = currentStok - jumlahDibeli;
              batch.update(docRef, {'stok': newStok < 0 ? 0 : newStok});
           } else {
              print("Warning: Product document ${docId} not found during stock update.");
           }
        }
      }
      await batch.commit();
      ...
      for (var item in finalKeranjangData) {
        int beli = item['harga_beli'] as int? ?? 0; // Get purchase price from cart item
        int jual = item['harga_jual'] as int? ?? 0;
        int jumlah = item['jumlah'] as int? ?? 0;
        if (jual > 0 && beli >= 0 && jumlah > 0) { // Ensure valid numbers
             totalProfit += (jual - beli) * jumlah;
        } else {
             print("Warning: Invalid price/quantity for profit calculation for item ${item['nama']}");
        }
      }
